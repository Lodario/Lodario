...
export async function POST(request: NextRequest) {
  if (!PUBLIC_BETA_FEATURES.guardianAndMinorAccounts) {
    return NextResponse.json({ error: PUBLIC_BETA_UNAVAILABLE_MESSAGE }, { status: 404 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: getSafeConfigurationMessage(error) }, { status: 500 });
  }
  const client = createClient(supabaseConfig.url, supabaseConfig.anonKey, { global: { headers: { Authorization: authorization } }, auth: { persistSession: false } });
  ...
  try {
    ...
    console.error('[guardian-invitation] SMTP delivery failed.');
